...
text = ~paste('Airmen: ', values)
ggplot(states, aes(text = ~paste('Airmen: ', n))) %>% 
...
trains <- small_trains %>% 
  ...
  labs(title = "Average Number of Trips per Month over a Year",
...
trains <- animate(trains, duration = 15)
anim_save("trains.gif", animation = trains)
```
...
garden <- animate(garden, duration = 20)
anim_save("garden.gif", animation = garden)
```
...
ggmap(panama_map) +
  ...
  geom_path(data = tri_panama, 
             ...
             size = .5) +
  #geom_point(data = mallorca_bike_day7,
  #           aes(x = lon, y = lat, group = seq_along(time)), colour = 'red', size = 1) +
  #scale_color_viridis_c(option = "magma") +
  theme_map() +
  theme(legend.background = element_blank()) +
...
map
anim_save("map.gif", animation = map)
```

We see how number of covid cases reported cumulatively change from 2020 to 2021,
